...
**构建数据集：**
- 因为保存形式为 index 形式，需要根据词表构建映射 `word2index = {word: index for index, word in enumerate(vocab_list)}`
- 构建数据集
```python
...
##### torch 数据集
**定义 Dataset 类**
- 继承 torch 中的 Dataset 类，并实现 `__init__`, `__getitem__`, `__len__` 三个方法
**获取 DataLoader 方法**
- 通过方法获取 DataLoader，根据参数控制测试集和训练集
```python
...
![[Pasted image 20260704155219.png]]
**Embedding 层**
- 方式 1：随机初始化 `self.embedding = nn.Embedding()`
- 方式 2：从预训练模型中加载
